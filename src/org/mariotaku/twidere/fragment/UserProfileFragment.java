...
import org.mariotaku.twidere.dialog.ClipboardDialog;
...
public class UserProfileFragment extends BaseSupportListFragment implements OnClickListener,
    ...
    private View mNameContainer, mDescriptionContainer, mLocationContainer, mURLContainer,
            mTweetsContainer,
            mFollowersContainer, mFriendsContainer, mEditFollowView, mEditFollowLabelContainer,
            mMoreOptionsButton, mCreateAtContainer;
    ...
    private final LoaderCallbacks<SingleResponse<ParcelableUser>> mUserInfoLoaderCallbacks = new LoaderCallbacks<SingleResponse<ParcelableUser>>() {
        ...
        public Loader<SingleResponse<ParcelableUser>> onCreateLoader(final int id, final Bundle args) {
            ...
            final boolean omit_intent_extra = args != null ? args.getBoolean(
                    EXTRA_OMIT_INTENT_EXTRA, true) : true;
    ...
    private final LoaderCallbacks<SingleResponse<Relationship>> mFriendshipLoaderCallbacks = new LoaderCallbacks<SingleResponse<Relationship>>() {
        ...
        public void onLoadFinished(final Loader<SingleResponse<Relationship>> loader,
            ...
            mEditFollowView.setVisibility(data.data == null && !user_is_me ? View.GONE
                    : View.VISIBLE);
        }
    ...
    public void displayUser(final ParcelableUser user) {
        ...
        mDescriptionContainer.setVisibility(user_is_me || !isEmpty(user.description_html) ? View.VISIBLE
                : View.GONE);
        mDescriptionView.setText(user.description_html != null ? Html.fromHtml(user.description_html)
                : null);
        ...
        switch (view.getId()) {
            ...
            case R.id.more_options: {
                ...
                if (mUser.id != mAccountId) {
                    ...
                    final boolean is_following_me = mFriendship != null
                            && mFriendship.isTargetFollowingSource();
    ...
    public View onCreateView(final LayoutInflater inflater, final ViewGroup container,
        ...
        mCreateAtContainer = mHeaderView.findViewById(R.id.created_at_container);
    ...
    final class UserListMembershipsAction extends ListAction {
        ...
        public String getName() {
            ...
            final String display_name = getDisplayName(getActivity(), mUser.id, mUser.name,
                    mUser.screen_name);
    ...
    final class UserListsAction extends ListAction {
        ...
        public String getName() {
            if (mUser == null) {
                return getString(R.string.users_lists);
            }
            final String display_name = getDisplayName(getActivity(), mUser.id, mUser.name,
                    mUser.screen_name);
    ...
    private final OnLongClickListener mClipboardListener = new OnLongClickListener() {
        ...
        public boolean onLongClick(View v) {
            ...
            switch (v.getId()) {
                case R.id.name_container:
                    text = mNameView.getText() + "\n" + mScreenNameView.getText();
                    break;
                case R.id.description_container:
                    text = mDescriptionView.getText();
                    break;
                case R.id.location_container:
                    text = mLocationView.getText();
                    break;
                case R.id.url_container:
                    text = mURLView.getText();
                    break;
                case R.id.created_at_container:
                    text = mCreatedAtView.getText();
                    break;
                default:
                    break;
            }
            if (!isEmpty(text)) {
                ClipboardDialog.show(getActivity(), text.toString());
                return true;
            }
            return false;
        }
    ...
    private void attachClipboardListeners() {
        mNameContainer.setOnLongClickListener(mClipboardListener);
        mDescriptionContainer.setOnLongClickListener(mClipboardListener);
        mLocationContainer.setOnLongClickListener(mClipboardListener);
        mURLContainer.setOnLongClickListener(mClipboardListener);
        mCreateAtContainer.setOnLongClickListener(mClipboardListener);
    }
